...
- 新增可選吉祥物架構：小黑仍是預設角色，並新增原創角色「小拳」作為可指定的替代吉祥物。
...
本版也支援可選原創吉祥物「小拳」。小拳是一個極簡、冷靜、用最小動作解決複雜問題的原創角色，適合用來表達「直接切入核心」、「拆掉阻塞」、「把複雜問題壓成一個可行動作」這類內文隱喻。小拳不是任何既有漫畫、動畫、遊戲或品牌角色的複製、改作或圖像引用；這個 repo 不放入既有作品的漫畫圖檔，也不使用既有角色的服裝、臉部特徵、名稱或世界觀。
...
如果使用者指定「小拳」，整體風格仍維持純白、手繪、留白與少量紅橙藍批註，只是把核心動作主體從小黑換成小拳。小拳要像白板草圖裡的問題拆解員，而不是英雄海報或戰鬥漫畫角色。
...
示例圖來源規則：README 與 skill 示例圖必須逐張由 `image_gen` 生成。PIL、SVG、Canvas、HTML、Matplotlib、Graphviz、自寫繪圖腳本或流程圖工具只能用於檔案處理，不能用來產生示例圖內容。若要重畫示例，必須逐張呼叫 `image_gen`，並在示例資料夾留下來源紀錄。
...
6. 讓指定吉祥物承擔核心動作；未指定時使用小黑
7. 每張圖單獨呼叫影像模型生成
8. 按 QA checklist 檢查：白底、留白、吉祥物動作、中文標註、非 PPT 感、非舊案例復刻
9. 儲存最終 PNG，並報告用途和路徑
...
- 使用小拳時，也必須讓小拳承擔核心動作；如果去掉小拳畫面仍然完全成立，說明小拳太裝飾了。
- 小拳是原創可選角色，不應使用任何既有漫畫、動畫、遊戲或品牌角色的圖片、服裝、臉部特徵、名稱、標誌或世界觀。
- 範例圖只用於校準線條密度、留白、顏色剋制和吉祥物參與方式，不要復刻構圖。
